...
public class BurrowsWheeler {
	...
	private final static int IHuffConstants_ALPH_SIZE = 127;
     ...
     *
     */
    private class Rotatable implements Comparable<Rotatable>{
    ...
    public BurrowsWheeler(){
        myIndexes = new int[IHuffConstants_ALPH_SIZE];
        myFirst = -1;
        resetIndexes();
    }
...
//    }
     ...
     */
    private char[] decode(char[] list, int first){
       
        int[] translate = new int[list.length];
        int[] sums = new int[IHuffConstants_ALPH_SIZE + 1];
        for(int k=0; k < list.length; k++){
            sums[list[k]+1]++;
        }
